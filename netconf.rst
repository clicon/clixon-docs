...
.. note::
   ...
   since peer OS identity is obtained from socket credentials. This is default.

External groups
^^^^^^^^^^^^^^^
RFC 8341 Sec 3.3.4.5 defines the ``enable-external-groups`` leaf
(default ``true``).  The group names of the "transport layer for a session" are used
together with the locally configured NACM group entries to determine
which NACM groups the user belongs to.

Clixon implements this by looking up all OS groups of the peer user (primary and
supplementary) at connection time ``getgrent(3)``.  For each OS group whose
name matches a NACM ``groups/group/name``, that group is included in the user's
effective NACM group set — even if the username does not appear in the group's
``user-name`` list.

Example: if the OS user ``alice`` is a member of the OS group ``operators``, and the
NACM configuration contains::
...
then ``alice`` will automatically be a member of the NACM ``operators`` group and
inherit any rules associated with it, as long as ``enable-external-groups`` is
``true``.
